...
impl<'a> RelayClient<'a> {
    ...
    /// POST an envelope to a recipient's inbox. The body is opaque bytes
    /// (markdown with frontmatter, body sealed).
    pub async fn send(
        &self,
        recipient: &Did,
        body: &[u8],
        ...
        let r = self
            .http
            .post(format!("{}/v0/inbox/{}", self.endpoint, recipient.as_str()))
    ...
    /// Pull all envelopes with id > `after`. Caller maintains the cursor in
    /// [`RelayState`] and bumps it after successfully ingesting each batch.
    pub async fn poll(
        &self,
        token: &str,
        after: u64,
        ...
        let r = self
            .http
            .get(format!("{}/v0/inbox/{}", self.endpoint, self.did.as_str()))
            .query(&[("after", after)])
