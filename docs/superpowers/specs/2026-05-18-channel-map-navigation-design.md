...
* Pinch / scroll-zoom *while pointer is over the chain* = time density (six months across screen ↔ one day zoomed in). Pinch elsewhere on the viewport = camera zoom (the standard map gesture). The chain owns its own zoom axis to keep time-walk and camera-walk separable.
...
| 4. Peripheral Presence | ✅ Strong | No notifications; ambient pulse + glow; map is *gone to*, not demanding |
