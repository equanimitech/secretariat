...
- Recontextification: Secretariat is *messaging* + *Zettelkasten on the same primitive*. The same envelope can be a fleeting capture in `inbox:triage`, get promoted to a permanent note in `area:writing`, and eventually become an outbound letter to a peer (Marcelo, Christophe). The graph of replies + quote-references between envelopes — local + remote — is the principal's knowledge graph.
...
- Questions:
  - Should the substrate explicitly recognize a "Zettel" classification, or is `Recipient::LocalQueue` enough? Lean: recipient is enough; Zettelkasten is a *use pattern* of the substrate, not a new primitive.
