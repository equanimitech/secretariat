...
Pitch — 2026-05-18. Source: free-text follow-up to \[\[project_secretariat_agent]] and \[\[project_daemon_v03_subsystems]] (2026-05-18 session)
...
_When:_ every morning (or whatever cadence the principal sets) the daemon ticks, agent runs, digest envelope appears at `_self/.../inbox/`, signed-only (not stamped — per \[\[project_stamp_is_selective_weight]]). Baseline today: nothing. Principal either does a manual `sec review` walk or skips entirely.
